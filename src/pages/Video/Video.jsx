...
const Video = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const videoRes = await axios.get(`${REACT_APP_BASE_URL}/videos/find/${path}`);
        const channelRes = await axios.get(
          `${REACT_APP_BASE_URL}/users/find/${videoRes.data.userId}`
        );
  ...
  useEffect(() => {
    const fetchView = async () => {
      try {
        await axios.put(
          `${REACT_APP_BASE_URL}/videos/view/${path}`,
          {},
          {
  ...
  const handleLike = async () => {
    await axios.put(
      `${REACT_APP_BASE_URL}/users/like/${currentVideo?._id}`,
      {},
      {
        headers: { Authorization: localStorage.getItem('Authorization') },
      }
  ...
  const handleDislike = async () => {
    await axios.put(
      `${REACT_APP_BASE_URL}/users/dislike/${currentVideo?._id}`,
      {},
      {
        headers: { Authorization: localStorage.getItem('Authorization') },
      }
  ...
  const handleSub = async () => {
    currentUser?.user?.subscribedUsers?.includes(channel?._id)
      ? await axios.put(
          `${REACT_APP_BASE_URL}/users/unsub/${channel?._id}`,
          {},
          {
            headers: { Authorization: localStorage.getItem('Authorization') },
          }
      ...
      : await axios.put(
          `${REACT_APP_BASE_URL}/users/sub/${channel?._id}`,
          {},
          {
            headers: { Authorization: localStorage.getItem('Authorization') },
          }
